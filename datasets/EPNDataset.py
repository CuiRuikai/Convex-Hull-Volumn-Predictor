...
class EPN3D(data.Dataset):
    ...
    def _get_transforms(self, subset):
        if subset == 'train':
            return data_transforms.Compose([{
                ...
                'callback': 'PointcloudRotate',
                'objects': ['pcloud']
            },{
                'callback': 'PointcloudScale',
                'objects': ['pcloud']
            },{
                'callback': 'ToTensor',
                'objects': ['pcloud']
    ...
    def _get_file_list(self, subset):
        ...
        for dc in self.dataset_categories:
            ...
            for (partial_file, complete_file) in zip(partial_samples, complete_samples):
                ...
                file_list['model_id'].append(complete_file)
                file_list['file_path'].append(self.partial_points_path % (category_name, partial_file))
                file_list['file_path'].append(self.complete_points_path % (category_name, complete_file))

        print_log('Complete collecting files of the dataset. Total files: %d' % len(file_list['file_path']), logger='EPN3DDATASET')
        return file_list
    ...
    def __getitem__(self, idx):
        ...
        data['pcloud'] = IO.get(self.file_list['file_path'][idx]).astype(np.float32)

        if self.transforms is not None:
            data = self.transforms(data)
        data['vol'] = torch.from_numpy(np.array(ConvexHull(data['pcloud']).volume)).float()

        return sample['taxonomy_id'], sample['model_id'], (data['pcloud'], data['vol'])

    def __len__(self):
        return len(self.file_list['file_path'])