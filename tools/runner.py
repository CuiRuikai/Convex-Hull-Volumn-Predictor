...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if dataset_name == 'EPN3D':
                pcloud = data[0].cuda()
                hull_vol = data[1].cuda()
            else:
                raise NotImplementedError(f'Train phase do not support {dataset_name}')

            num_iter += 1

            preds = base_model(pcloud)
            loss = loss_func(preds.flatten(), hull_vol)
            loss.backward()
            ...
            if idx % 100 == 0:
                print_log('[Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.6f (s) Losses = %s lr = %.6f' %
                            ...
                            ['%.8f' % l for l in losses.val()], optimizer.param_groups[0]['lr']), logger = logger)
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            ret = base_model(pcloud)

            _metrics = Metrics.get(ret.flatten(), vol)
            ...
            if (idx+1) % 1000 == 0:
                print_log('Test[%d/%d] Taxonomy = %s Sample = %s Metrics = %s' %
                            (idx + 1, n_samples, taxonomy_id, model_id, ['%.8f' % m for m in _metrics]), logger=logger)


        for _,v in category_metrics.items():
            test_metrics.update(v.avg())
        print_log('[Validation] EPOCH: %d  Metrics = %s' % (epoch, ['%.8f' % m for m in test_metrics.avg()]), logger=logger)
